...
histDirectional <- function( x, k, p=2, plot.type="default", freq=TRUE,
                              positive.only=FALSE, report="summary", label.orthants=TRUE, 
...
invisible( list(counts=tally$counts, nrejects=tally$nrejects, nties=tally$nties,
           ...
           plot.type=plot.type,report=report)) }
           # modified 30 Oct 2016 to also return which simplex
###############################################################################
TallyHrep <- function( x, H, report="summary" ) {
# tally the number of x vectors that are in the simplices specified by H
#
# Input values:
#   x a (n x d) data matrix, rows x[1,],...,x[n,] are data points (d-dim vectors)
#   H a (m x (d+2) x nbins) array, H[,,j] is the H-representation of the j-th simplex,
#      with m=number of linear constraints 
